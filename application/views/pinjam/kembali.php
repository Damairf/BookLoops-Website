							...
							<div class="col-sm-7">
								<table class="table table-striped">
									...
									<tr>
										...
										// Tanggal sekarang dan tanggal kembali yang diharapkan
										$date1 = date('Y-m-d');
										$date2 = $pinjam->tgl_balik; // Pastikan format tgl_balik dalam format Y-m-d

										// Hitung selisih hari
										$diff = (strtotime($date1) - strtotime($date2)) / (60 * 60 * 24); // Selisih dalam hari
										$diff = max(0, $diff); // Pastikan tidak ada nilai negatif

										if ($diff > 0) {
											// Ambil biaya denda per hari yang aktif
											$dd = $this->M_Admin->get_tableid_edit('tbl_biaya_denda', 'stat', 'Aktif'); 
											if ($dd) {
												// Hitung total denda berdasarkan jumlah buku
												$total_harga_denda = $dd->harga_denda * $diff * $jml_buku;

												// Tampilkan total harga denda dengan warna merah
												echo '<span style="color:red;">' . $this->M_Admin->rp($total_harga_denda) . '</span>';
											} else {
												// Jika denda belum diatur
												echo '<p style="color:orange;">Denda belum diatur</p>';
											}
										} else {
											// Jika tidak ada keterlambatan
											echo '<p style="color:green;text-align:center;">Tidak Ada Denda</p>';
										}
										?>
										</td>
									</tr>
...
<div id="modal_body" class="modal-body fileSelection1">
	<table class="table table-striped">
		...
		<tr>
			...
			<?php
			$date1 = date('Y-m-d');
			$date2 = $pinjam->tgl_balik;
			
			$diff = (strtotime($date1) - strtotime($date2)) / (60 * 60 * 24);
			$diff = max(0, floor($diff));
			
			if ($diff > 0) {
				echo $diff . ' hari';
			} else {
				echo '<p style="color:green;text-align:center;">Tidak Ada Keterlambatan</p>';
			}
			?>			
			</td>
		</tr>
		...
		<tr>
			...
			// Tanggal sekarang dan tanggal kembali yang diharapkan
			$date1 = date('Y-m-d'); // Tanggal sekarang
			$date2 = $pinjam->tgl_balik; // Tanggal kembali yang diharapkan

			// Hitung selisih hari
			$diff = (strtotime($date1) - strtotime($date2)) / (60 * 60 * 24); // Selisih dalam hari
			$diff = max(0, floor($diff)); // Pastikan nilai minimal 0 dan tanpa desimal
			...
			if ($diff > 0) {
				...
				if ($dd) {
					...
					// Jika aturan denda tidak ditemukan
					echo '<p style="color:orange;">Denda belum diatur</p>';
				}
			} else {
				// Jika tidak ada keterlambatan
