                ...
                <tbody>
                  ...
                  <tr>
					...
					if ($isi['status'] == 'Di Kembalikan') {
						echo isset($total_denda->denda) ? $this->M_Admin->rp($total_denda->denda) : 'Tidak ada denda';
					} else {
						...
						if ($diff > 0) {
							...
							if ($dd) {
								$total_harga_denda = $jml * ($dd->harga_denda * $diff);

								echo '<p style="color:red;font-size:18px;">' . $this->M_Admin->rp($total_harga_denda) . '</p>';
