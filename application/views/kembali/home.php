						...
						?>
                            <tr>
								...
								<?php 
									...
									// Periksa apakah ada data denda sebelumnya
									if ($denda->num_rows() > 0) {
										$s = $denda->row();
										// Tampilkan denda yang sudah tercatat
										echo $this->M_Admin->rp($s->denda);
									} else {
										// Ambil tanggal sekarang dan tanggal pengembalian yang diharapkan
										$date1 = date('Ymd');
										$date2 = preg_replace('/[^0-9]/', '', $isi['tgl_balik']);
										$diff = $date2 - $date1; // Selisih antara tanggal kembali yang diharapkan dan sekarang

										// Jika tidak ada keterlambatan
										if ($diff >= 0) {
											echo '<p style="color:green;">Tidak Ada Denda</p>';
										} else {
											// Jika terlambat, ambil harga denda yang aktif
											$dd = $this->M_Admin->get_tableid_edit('tbl_biaya_denda', 'stat', 'Aktif');
											if ($dd) {
												// Hitung denda total berdasarkan jumlah buku dan hari keterlambatan
												$total_denda = abs($diff) * $dd->harga_denda * $jml;

												// Tampilkan total denda dalam format mata uang dan warna hitam
												echo '<p style="color:black; font-size:18px;">' . $this->M_Admin->rp($total_denda) . '</p>';
												echo '<small style="color:#333;">* Untuk ' . $jml . ' Buku</small>';
											}
										}
									}
								?>
