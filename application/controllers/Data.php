...
class Data extends CI_Controller {
	...
	public function bukudetail()
	{
		...
		if($count > 0)
		{
			...
			echo '<script>alert("KOLEKSI TIDAK DITEMUKAN");window.location="'.base_url('data').'"</script>';
		}

		$this->data['title_web'] = 'Detail Koleksi';
        $this->load->view('header_view',$this->data);
        $this->load->view('sidebar_view',$this->data);
	...
	public function bukuedit()
	{
		...
		$this->data['title_web'] = 'Edit Detail Koleksi';
        $this->load->view('header_view',$this->data);
        $this->load->view('sidebar_view',$this->data);
	...
	public function bukutambah()
	{
        ...
        $this->data['title_web'] = 'Tambah Koleksi';
        $this->load->view('header_view',$this->data);
        $this->load->view('sidebar_view',$this->data);
	...
	public function prosesbuku()
	{
		...
		if(!empty($this->input->post('tambah')))
		{
			...
			if ($buku) {
				...
				$new_jml = $buku->jml + htmlentities($post['jml']);
				$this->db->where('buku_id', $buku_id);
				$this->db->update('tbl_buku', ['jml' => $new_jml]);
			} else {
				$data = array(
					'buku_id'=>$buku_id,
					'id_kategori'=>htmlentities($post['kategori']), 
					'id_rak' => htmlentities($post['rak']), 
					'buku_id' => htmlentities($post['buku_id']),
					'isbn' => htmlentities($post['isbn']), 
					'no_klasifikasi' => htmlentities($post['no_klasifikasi']), 
	...
	public function rak()
	{
        ...
        $this->data['title_web'] = 'Data Lokasi Koleksi ';
        $this->load->view('header_view',$this->data);
        $this->load->view('sidebar_view',$this->data);
