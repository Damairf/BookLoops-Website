...
class Transaksi extends CI_Controller {
	...
	public function prosespinjam()
	{
		...
		if (!empty($post['tambah'])) {
			$tgl = $post['tgl'];
			$lama = $post['lama'];
			$tgl2 = date('Y-m-d', strtotime('+' . $lama . ' days', strtotime($tgl)));

			$hasil_cart = array_values(unserialize($this->session->userdata('cart')));
			$stok_cukup = true;

			foreach ($hasil_cart as $isi) {
				$stok = $this->db->query("SELECT jml FROM tbl_buku WHERE buku_id = ?", [$isi['id']])->row();
				if (!$stok) {
					log_message('error', "Buku dengan ID {$isi['id']} tidak ditemukan.");
					$this->session->set_flashdata('pesan', '<div id="notifikasi"><div class="alert alert-danger">
					<p> Buku tidak ditemukan di database!</p>
					</div></div>');
			...
			$stok = $stok->jml;
            log_message('debug', "Stok buku dengan ID {$isi['id']}: {$stok}");

            if ($stok < $isi['qty']) {
                log_message('debug', "Stok tidak mencukupi untuk buku ID {$isi['id']}. Dibutuhkan: {$isi['qty']}, tersedia: {$stok}");
                $stok_cukup = false;
            }

			if (!$stok_cukup) {
				$this->session->set_flashdata('pesan', '<div id="notifikasi"><div class="alert alert-danger">
				<p> Stok buku tidak mencukupi untuk peminjaman!</p>
				</div></div>');
				redirect(base_url('transaksi'));
				return;
			}
			...
			$this->db->trans_start();
			foreach ($hasil_cart as $isi) {
				$this->db->query("UPDATE tbl_buku SET jml = jml - ? WHERE buku_id= ?", [$isi['qty'], $isi['id']]);
				$data[] = array(
			...
			if (!empty($data)) {
				...
				$this->session->set_userdata('cart', serialize([]));
			}

			$this->db->trans_complete();

			if ($this->db->trans_status() === FALSE) {
				log_message('error', "Transaksi gagal untuk pinjam ID {$post['nopinjam']}.");
				$this->session->set_flashdata('pesan', '<div id="notifikasi"><div class="alert alert-danger">
				<p> Terjadi kesalahan saat memproses data!</p>
				</div></div>');
				redirect(base_url('transaksi'));
				return;
			}

			log_message('info', "Transaksi berhasil untuk pinjam ID {$post['nopinjam']}.");
		...
		if ($this->input->get('kembali')) {
			...
			foreach ($pinjam->result_array() as $isi) {
				...
				$buku_id = $isi['buku_id'];
				$qty = 1; // Atur nilai qty sesuai logika Anda jika memungkinkan ada lebih dari 1 buku per transaksi
		
				// Periksa denda
				$denda = $this->db->query("SELECT * FROM tbl_denda WHERE pinjam_id = ?", [$pinjam_id]);
				$jml_buku = $this->db->query("SELECT COUNT(*) as jumlah FROM tbl_pinjam WHERE pinjam_id = ?", [$pinjam_id])->row()->jumlah;
				$jml = $pinjam->num_rows();
				if ($denda->num_rows() > 0) {
					$s = $denda->row();
					echo $s->denda;
				} else {
					$tgl_balik = new DateTime($isi['tgl_balik']);
				$tgl_sekarang = new DateTime();

				// Hitung selisih hari dengan PHP DateTime
				$selisih_hari = $tgl_balik->diff($tgl_sekarang)->days;

				// Periksa apakah terlambat (tanggal pengembalian lebih dari tgl_balik)
				if ($tgl_sekarang > $tgl_balik) {
					$dd = $this->M_Admin->get_tableid_edit('tbl_biaya_denda', 'stat', 'Aktif');
					$harga_denda = $selisih_hari * $dd->harga_denda * $jml_buku;
					$lama_waktu = $selisih_hari;
				} else {
					$harga_denda = 0;
					$lama_waktu = 0;
				}
				...
				$this->db->query("UPDATE tbl_buku SET jml = jml + ? WHERE buku_id = ?", [$qty, $buku_id]);
		
				// Tambahkan data denda
				$data_denda = array(
					'pinjam_id' => $pinjam_id,
					'denda' => $harga_denda,
